...
function checkAndAddListeners() {
    var elements = [
        ...
        { selector: '#btnDistort', event: 'click', handler: applyNoiseFilter }, 
...
document.querySelector('#btnDistort').addEventListener('click', async () => {
    try {
...
async function applyNoiseFilter() {
    ...
    await core.executeAsModal(async () => {
        ...
        var currentLayer = doc.activeLayers[0];

        // Check if a layer is selected
        if (!currentLayer) {
            throw new Error("No active layer selected. Please select a layer to apply the effect.");
        }

        // Duplicate the current layer to work non-destructively
        ...
        // Update reference to the newly duplicated layer
        var newLayer = photoshop.app.activeDocument.activeLayers[0];

        // Ensure the layer is rasterized if it's not a pixel layer
        if (!newLayer.isPixelLayer) {
            await photoshop.action.batchPlay([
                {
                    "_obj": "rasterizeLayer",
                    "_target": [{ "_ref": "layer", "_enum": "ordinal", "_value": "targetEnum" }]
        ...
        await photoshop.action.batchPlay([
            {
                "_obj": "addNoise",
                "amount": { "_unit": "percentUnit", "_value": 15 }, // Adjustable noise amount
        ...
        await photoshop.action.batchPlay([
            {
                ...
                "scaleHorizontal": 100,
                "scaleVertical": 100,
                "waveType": { "_enum": "waveType", "_value": "sine" },
        ...
        await photoshop.action.batchPlay([
            {
                "_obj": "hueSaturation",
                "saturation": 100, // Adjustable saturation
                "lightness": 0
            }
        ], { "synchronousExecution": true });

    }, { "commandName": "Apply Custom Effects" }); // Groups all actions under a single undo step
}
...
async function storeEncryptedMessageInMetadata(doc, encryptedMessage, key) {
    var binaryMessage = stringToBinary(encryptedMessage) + '48492648';
