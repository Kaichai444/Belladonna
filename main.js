...
document.querySelector('#btnDistort').addEventListener('click', async () => {
    try {
        await applyCustomEffects();
    } catch (error) {
        console.error("Error applying custom effects:", error);
    }
});

async function applyCustomEffects() {
    ...
    await core.executeAsModal(async () => {
        ...
        // Ensure the layer is rasterized (if necessary)
        if (!currentLayer.isPixelLayer) {
            await photoshop.action.batchPlay([
                {
                    "_obj": "rasterizeLayer",
                    "_target": [{ "_ref": "layer", "_enum": "ordinal", "_value": "targetEnum" }]
