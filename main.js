...
var { executeAsModal } = require("uxp").storage;
...
function getSelectedLayers() {
    ...
    try {
        ...
        console.log("Error retrieving selected layers: " + e.message);
    }
    return selectedLayers;
}


function importSelectedLayers() {
    var layers = getSelectedLayers();
    if (layers.length === 0) {
        alert("No layers selected! Please select layers to encrypt.");
        return;
    }
...
function derivePoisonLevel(key) {
    ...
    var poisonLevel = Math.abs(hash) % 100 + 1; // Ensure it's between 1 and 100
    return poisonLevel;
}

async function embedMessageInLayer(layer, encryptedMessage) {
    var binaryMessage = stringToBinary(encryptedMessage) + '00000000'; // Null terminator
    var pixels = await layer.pixelData;

    for (let i = 0, j = 0; i < pixels.length && j < binaryMessage.length; i += 4) {
        let red = pixels[i];  // Modify the red channel LSB
...
async function encryptSelectedLayers() {
    ...
    const encryptedMessage = await encryptMessage(secretMessage, encryptionKey);
...
document.querySelector('#embed-message').addEventListener('click', embedMessage);
document.querySelector('#retrieve-message').addEventListener('click', retrieveMessage);
document.querySelector('#save-image').addEventListener('click', saveEncryptedImage);
