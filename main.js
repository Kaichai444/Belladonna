...
async function exportAsPNGWithBlendModeEncoding(doc, message, key) {
    try {
        // Retrieve all layers and encode message
        var layers = await getAllLayers();
        await encodeMessageInBlendModes(layers, message, key);

        try {
            var file = await storage.localFileSystem.getFileForSaving("exported_image.png");
            if (!file) {
                throw new Error("File selection was canceled.");
            }
            // Proceed with PNG export using the selected file
        } catch (error) {
        ...
        if (saveFile) {
            const saveOptions = new core.SaveOptions({
                format: core.SaveOptions.Format.PNG,
                compression: 9
            });
