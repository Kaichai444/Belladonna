...
async function encryptSelectedLayers() {
    ...
    // Get encryption key and secret message
    var encryptionKey = document.getElementById('encryption-key').value;
    var secretMessage = document.getElementById('secret-message').value;
    ...
    try {
        ...
        await executeAsModal(async () => {
            var activeDocument = app.activeDocument;
            await embedMessageInMetadata(activeDocument, encryptedMessage); // Pass the necessary parameters
            await storeEncryptedMessageInMetadata(activeDocument, encryptedMessage, encryptionKey);
        ...
        console.log('Encryption and storage in metadata completed successfully.');
    } catch (error) {
...
async function exportAsPNGWithBlendModeEncoding(doc, message, key) {
    try {
        ...
        // Encode the message into layer blend modes
        await encodeMessageInBlendModes(layers, message, key);

        // Choose save location
        var folder = await core.fs.getFolder();
